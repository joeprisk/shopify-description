...
module.exports = (product) => {
    ...
    return new Promise((resolve) => {
        ...
        sections.map((text, i) => {
            ...
            text === 'description' && (bpProduct['Long description'] = $(`#${text}`).nextUntil(`#${next}`).html());
            text === 'specifications' && (bpProduct['PCF_SPECIFIC'] = $(`#${text}`).nextUntil(`#${next}`).html());
            text === 'features' && (bpProduct['PCF_FEATURES'] = $(`#${text}`).nextUntil(`#${next}`).html());
        });
